...
Grid.prototype.addStartTiles = function () {
  for (var i=0; i<this.startTiles; i++) {
    this.addRandomTile(startTileValue);
  }
};

// Adds a tile in a random position
Grid.prototype.addRandomTile = function (val) {
  if (this.cellsAvailable()) {
	
    var value=val;
	if (!val)
	{
		value= Math.random() < 0.9 ? randTileVal1 : randTileVal2;
	}
...
Grid.prototype.move = function (direction) {
  ...
  traversals.x.forEach(function (x) {
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && (next.value === tile.value || diffCanMerge) && !next.mergedFrom) {
          var merged = new Tile(positions.next, Math.max(tile.value, next.value)*multFactor);
	  ...
	  //if (removeOnClick){$(".tile").on("click",function(){manager.removeTile(Math.round(getComputedStyle(this).left/121),Math.round(getComputedStyle(this).top/121));})}
    });
  });
  ...
  //}
  //if (removeOnClick){deleteOnClick();}
...
Grid.prototype.islands = function() {
  ...
  var mark = function(x, y, value) {
    if (x >= 0 && x <= jSize-1 && y >= 0 && y <= jSize-1 &&
        self.cells[x][y] &&
  ...
  for (var x=0; x<jSize; x++) {
    for (var y=0; y<jSize; y++) {
      if (this.cells[x][y]) {
...
Grid.prototype.monotonicity2 = function() {
  ...
  for (var x=0; x<jSize; x++) {
    ...
    var next = current+1;
    while ( next<4 ) {
      while ( next<jSize && !this.cellOccupied( this.indexes[x][next] )) {
        next++;
      }
      if (next>=jSize) { next--; }
  ...
  for (var y=0; y<jSize; y++) {
    ...
    var next = current+1;
    while ( next<jSize ) {
      while ( next<jSize && !this.cellOccupied( this.indexes[next][y] )) {
        next++;
      }
      if (next>=jSize) { next--; }
  ...
  for (var x=0; x<jSize; x++) {
    for (var y=0; y<jSize; y++) {
      if (self.cellOccupied(this.indexes[x][y])) {
        if (self.cellContent(this.indexes[x][y]).value>=winValue){//== 2048) {
          return true;
        }
      }
    }
